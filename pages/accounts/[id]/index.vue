...
<template>
    ...
    <div v-else>
        <div class="flex items-baseline mb-2">
            <h1 class="text-5xl basis-full">{{ account.name }} ...{{ account.accountNumber }}</h1>
        ...
        <div class="flex justify-end gap-x-2 gap-y-1 flex-wrap">
            ...
            <FlexBreak />
            <div class="join">
                <button @click="createTransaction()" class="btn btn-outline btn-sm btn-primary w-fit join-item">
                ...
                <button v-for="transactionType in defaultTransactions" @click="createTransaction(transactionType)"
                    class="btn btn-outline btn-sm btn-primary w-fit join-item">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
